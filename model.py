...
class VGGBase(nn.Module):
    ...
    def __init__(self):
        ...
        torch.nn.init.xavier_uniform_(self.conv6.weight, gain=1)
        torch.nn.init.constant_(self.conv6.bias, 0.1)

        self.conv7 = nn.Conv2d(1024, 1024, kernel_size=1)
        torch.nn.init.xavier_uniform_(self.conv7.weight, gain=1)
        torch.nn.init.constant_(self.conv7.bias, 0.1)
...
class AuxiliaryConvolutions(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv10_2 = nn.Conv2d(128, 256, kernel_size=3, stride=2, padding=1)  # dim. reduction because stride > 1
...
class SSD300(nn.Module):
    ...
    def __init__(self, n_classes):
        ...
        self.pred_convs = PredictionConvolutions(n_classes)#98112个框（输入图像变为1024*1024）
    ...
    def create_prior_boxes(self):
        ...
        4、6、6、6、4、4
        64*64*
        128*128*4+64*64*6+32*32*6+16*16*6+8*8*4+4*4*4=98112
        """
        fmap_dims = collections.OrderedDict()
        fmap_dims['conv4_3'] = 128
        fmap_dims['conv7'] = 64
        fmap_dims['conv8_2'] = 32
        fmap_dims['conv9_2'] = 16
        fmap_dims['conv10_2'] = 8
        fmap_dims['conv11_2'] = 4
        ...
        prior_boxes.clamp_(0, 1)  # (98112, 4)
        return prior_boxes
    ...
    def detect_objects(self, predicted_locs, predicted_scores, min_score, max_overlap, top_k):
        ...
        """
        # set_trace()
        batch_size = predicted_locs.size(0)                 #获取batch大小
        n_priors = self.priors_cxcy.size(0)                     # 获取初始框个数
