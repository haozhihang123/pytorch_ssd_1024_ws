...
def transform(image, boxes, labels, difficulties, split):
    ...
    new_image, new_boxes = resize(new_image, new_boxes, dims=(1024, 1024))
...
def show_train_pic(images,boxes,i):
    ...
    for j in range(img_np.shape[0]):
        ...
        for box in boxes[j]:
            ...
            x = (1024*box).int()[0].item()
            y = (1024*box).int()[1].item()
            x1 = (1024*box).int()[2].item()
            y1 = (1024*box).int()[3].item()
            img_PIL = Image.open(save_dir+'/{}.jpg'.format(j))
            b=ImageDraw.ImageDraw(img_PIL)
            for i in range(3):
                b.rectangle((x+i,y+i,x1-i,y1-i),fill=None,outline='green')
            img_PIL.save(save_dir+'/{}.jpg'.format(j))
            print(x,y,x1,y1)
            img_PIL.show()
    # set_trace()

def show_eval_pic(image_dir,boxes,i,det_labels_batch,det_scores_batch):
    """ 
    展示训练图像并保存
    :param image_dir: 原图地址
    :param boxes: 真值框
    :param i: 批次信息
    :det_labels_batch:标签
    :det_scores_batch:分数
    ...
    for i,box in enumerate(boxes[0]) :
        ...
        x1 = (1024*box).int()[2].item()
        y1 = (1024*box).int()[3].item()
        label = rev_label_map[det_labels_batch[0][i].item()]
        score = det_scores_batch[0][i].item()
        b=ImageDraw.ImageDraw(img_PIL)
        for i in range(3):
            b.rectangle((x+i,y+i,x1-i,y1-i),fill=None,outline='green')
        a=ImageDraw.ImageDraw(img_PIL)
        a.text([x+5, y+5], '{}:{:.3f}'.format(label,score), (255,0,0))
        # img_PIL.save(save_dir+'/{}.png'.format(i))
        # print(x,y,x1,y1)
    # img_PIL.show()
# set_trace()
